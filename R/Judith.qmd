...
rhc_clean |> 
  ...
  mutate(percent = count / sum(count) * 100) |> 
  ungroup() |> 
  filter(swang1 == "No RHC") |> 
  select(age_group, percent)
```
This results suggests that younger patients (10-20 age group) and older patients (> 90 age group) may have higher rates of RHC procedures, potentially due to specific cardiac conditions or clinical indications. Conversely, middle-aged patients (40-50 and 60-70 age groups) may have lower rates of RHC procedures, indicating a different clinical approach or lower prevalence of conditions requiring RHC. Further research would be needed to understand the underlying reasons for these variations and fully evaluate the appropriateness of RHC procedures.
...
rhc_clean |> 
  ...
  select(sex, percent)
```
This suggest a potential difference in RHC utilization between females and males, with females having a slightly higher percentage of RHC procedures. Further research would be needed to fully understand and check the underlying reasons for these variations. 
...
```{r}
# Bar plot for Death by income and medical assurance
rhc_clean |>
  filter(death == 0) |> 
  ggplot(aes(x = income, 
